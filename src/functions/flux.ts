...
export default async function (
  ...
  function iteract() {
    ...
    socket.sendMessage(`${cliente.numero}@s.whatsapp.net`, {
      text: Eval(cliente, questions[cFlux].pergunta),
    });
  }
  function contraria(answer?: any) {
    ...
    socket.sendMessage(`${cliente.numero}@s.whatsapp.net`, {
      text: Eval(cliente, questions[cFlux].contraria),
    });
  ...
  socket.ev.on("messages.upsert", async (msg: any) => {
    ...
    if (number == cliente.numero && !webMessage.key.fromMe) {
      ...
      if (questions[cFlux].contraria.length < 2 && finish == false) {
        ...
        finish = true;
        return sendText(
          `Respostas de ${obj.cliente}:\n${cliente.numero} \n${obj.dados.map(
            (x: any) => {
              return `🟢${x.pergunta}: ${x.resposta}\n\n`;
            }
          )}`.replace(/,/g, "")
        );
      }
